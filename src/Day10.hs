...
import AdventData (day10, day10ex01, day10ex02, day10ex03, day10ex04, day10ex05, day10ex06)
import Data.List (group, groupBy, sort, sortOn, transpose)
...
rayOf (tx, ty) (x, y)
    | x == tx && y == ty = Nothing
rayOf (tx, ty) (x, y) =
    let angle = atan2 (fromIntegral (ty - y)) (fromIntegral (tx - x))
     in Just (quadOf angle, angle, abs (tx - x) + abs (ty - y), (x, y))
...
cellsFromRaw raw = concat $ loop 0 $ lines raw
  where
    cellsFromRawRow :: Int -> Int -> String -> [Pos]
    cellsFromRawRow _ _ [] = []
    cellsFromRawRow y x (c:rest)
        | c == '#' || c == 'X' = (x, y) : cellsFromRawRow y (x + 1) rest
    cellsFromRawRow y x (_:rest) = cellsFromRawRow y (x + 1) rest
    -- | loop over a set of lines
    loop :: Int -> [String] -> [[Pos]]
    loop _ [] = []
    loop y (row:rest) = cellsFromRawRow y 0 row : loop (y + 1) rest
...
solve raw =
    let g = cellsFromRaw raw
     in let counts = sort $ map (visCount g) g
         in counts !! max 0 (length counts - 1)
