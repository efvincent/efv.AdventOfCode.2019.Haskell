...
binOp world op params =
    world
        {mem = writeMem (fromInteger resultAddr) (fn v1 v2) (mem world), offset = offset world + 4}
  where
...
writeOp world param = world {outs = outs world ++ [toVal world param], offset = offset world + 2}

jumpOp :: World -> Op -> [ValueInstruction] -> World
jumpOp world op params = world {offset = newOffset}
  where
    (v1, v2) = (toVal world (head params), toVal world (params !! 1))
    fn =
        if op == JumpF
...
boolOp world op params = world {mem = writeMem addr newVal (mem world), offset = offset world + 4}
  where
    (v1, v2) = (toVal world (head params), toVal world (params !! 1))
...
adjRBaseOp world vi =
    let rb = rbase world + toVal world vi
