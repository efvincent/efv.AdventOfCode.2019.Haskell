...
solve raw from to =
    let s = State {path = [], dead = [], to = to, om = makeOrbitMap raw}
     in let (Just a) = checkCandidates [from] s
         in length (path a) - 2 -- -2 b/c we discount the actual start and destination
