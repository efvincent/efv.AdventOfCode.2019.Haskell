...
instToPath ins =
  ...
  where
    ...
    -- with the next line
    loop (('U',steps):xs) ((x,y),t) = [((x, y+step),t+step) | step <- [1..steps]] ++ loop xs ((x,y+steps),t+steps)
    loop (('D',steps):xs) ((x,y),t) = [((x, y-step),t+step) | step <- [1..steps]] ++ loop xs ((x,y-steps),t+steps)
    loop (('L',steps):xs) ((x,y),t) = [((x-step, y),t+step) | step <- [1..steps]] ++ loop xs ((x-steps,y),t+steps)
    loop (('R',steps):xs) ((x,y),t) = [((x+step, y),t+step) | step <- [1..steps]] ++ loop xs ((x+steps,y),t+steps)
