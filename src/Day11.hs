...
makeState startColor mem = State { world     = IC.setMemory IC.initialComputer mem
                                 , position  = (0, 0)
                                 , direction = UP
...
curColor s = ccol where (ccol, _) = M.findWithDefault (0, 0) (position s) (panel s)

execute :: State -> State
execute startState = if runnable then execute updatedState else updatedState
  where
